...
library(nnls)
    ...
    df <- readxl::read_excel(skyline_output) |> #input$
            ...
            df <- df |>
                ...
                dplyr::mutate(Area = dplyr::case_when(
                    includeISOnly == "Yes" & any(Molecule == "RS") ~ Area / first(Area[Molecule == "RS"]), #input$
                    TRUE ~ Area
                )) |>
                dplyr::ungroup()
            ...
            df_blank_avg <- df |>
                ...
                dplyr::pull(AverageBlank)  # Convert to a single value

            # Create a logical vector for the condition
            apply_blank_subtraction <- blankSubtraction == "Yes" #input$

            # Mutate the DataFrame with blank subtraction and handle negative values
            df <- df |>
                dplyr::mutate(
                    Area = pmax(0, Area - if (apply_blank_subtraction) df_blank_avg else 0)
                ) |>
                dplyr::ungroup()
        ...
        shiny::observeEvent(input$go, {
            ...
            CPs_standards <- Skyline_output_filt |>
                dplyr::filter(`Sample Type` == "Standard",
                              ...
                              Note != "NA") |>
                dplyr::group_by(!!input$standardAnnoColumn, Molecule) |>
                dplyr::mutate(rel_int = Area/sum(Area)) |> #why is it needed? Maybe can be removed
                tidyr::nest() |>
                dplyr::mutate(models = map(data, ~lm(Area ~ `Analyte Concentration`, data = .x))) |>
                dplyr::mutate(coef = map(models, coef)) |>
                dplyr::mutate(Response_factor = map_dbl(models, ~ coef(.x)["`Analyte Concentration`"]))|> #get the slope
                dplyr::mutate(intercept = purrr::map(coef, pluck("(Intercept)"))) |>
                dplyr::mutate(rsquared = purrr::map(models, summary)) |>
                dplyr::mutate(rsquared = purrr::map(rsquared, pluck("r.squared"))) |>
                ...
                dplyr::mutate(Response_factor = if_else(rsquared < removeRsquared(), 0, Response_factor)) |>
                dplyr::mutate(Chain_length = paste0("C", stringr::str_extract(Molecule, "(?<=C)[^H]+"))) |>
                ...
                dplyr::ungroup() |>
                dplyr::group_by(!!input$standardAnnoColumn, Chain_length) |> #grouping by the selected Note
                dplyr::mutate(Sum_response_factor_chainlength = sum(Response_factor, na.rm = TRUE)) |>
                dplyr::ungroup()
            # For SCCPs
            CPs_standardsS <- CPs_standards |>
                dplyr::filter(stringr::str_detect(Note, "S-")) |>
                dplyr::mutate(Response_factor = if_else(Chain_length %in% c("C14", "C15", "C16", "C17", "C18", "C19", "C20", "C21", "C22", "C23", "C24", "C25", "C26", "C27", "C28", "C29", "C30"), 0, Response_factor))

            # For MCCPs
            CPs_standardsM <- CPs_standards |>
                dplyr::filter(stringr::str_detect(Note, "M-")) |>
                dplyr::mutate(Response_factor = if_else(Chain_length %in% c("C10", "C11", "C12", "C13", "C18", "C19", "C20", "C21", "C22", "C23", "C24", "C25", "C26", "C27", "C28", "C29", "C30"), 0, Response_factor))

            # For LCCPs
            CPs_standardsL <- CPs_standards |>
                dplyr::filter(stringr::str_detect(Note, "L-")) |>
                dplyr::mutate(Response_factor = if_else(Chain_length %in% c("C10", "C11", "C12", "C13", "C14", "C15", "C16", "C17"), 0, Response_factor))
            ...
            CPs_samples <- Skyline_output() |>  # Call the reactive with ()
                dplyr::filter(`Sample Type` == "Unknown",
                              ...
                              Molecule != "RS",
                              `Isotope Label Type` == "Quan") |>
                dplyr::mutate(Chain_length = stringr::str_extract(Molecule, "(?<=C)[^H]+") |> as.numeric()) |>  # Extract number after "C"
            ...
            CPs_standards_input <- CPs_standards |>
                dplyr::select(Molecule, !!input$standardAnnoColumn, Response_factor) |>
                tidyr::pivot_wider(names_from = !!input$standardAnnoColumn, values_from = "Response_factor")
            ...
            combined <- CPs_samples_input |>
                dplyr::right_join(CPs_standards_input, by = "Molecule")
            ...
            Deconvolution <- combined_sample |>
                dplyr::mutate(result = map(data, ~ perform_deconvolution(.x, combined_matrix)))
        ...
        Skyline_recovery <- shiny::reactive({
            ...
            if ("IS" %in% colnames(RECOVERY) & "RS" %in% colnames(RECOVERY)) {
                RECOVERY <- RECOVERY |>
                    dplyr::mutate(dplyr::across(c(IS, RS), ~replace_na(.x, 0)))  # Replace NAs with 0 for IS and RS
            } else {
